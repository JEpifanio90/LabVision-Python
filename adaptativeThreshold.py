    ...
    def threshold(self):
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                #print gray[0]
                maximus=max(gray[0],maximus)
                minimus=min(gray[0],minimus)
        print maximus," and ",minimus
        self.adaptative(maximus,minimus)

    def adaptative(self,maximus,minimus):
         ran=maximus/16
         for x in range(self.width):
            for y in range(self.height):
                gray = self.imag.getpixel((x,y))
                gray=gray[0]
                if gray>=minimus and gray <= ran:
                    if (ran/2)>gray:
                        gray=minimus
                    else:
                        gray=ran
         ...
         self.newImag.save("adaptativeThreshold.png",self.imag.format)
         #self.newImage.show()
         print "Done.."
